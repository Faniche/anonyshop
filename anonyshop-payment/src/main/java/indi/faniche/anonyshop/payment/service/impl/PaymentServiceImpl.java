...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Map<String, Object> checkAlipayPayment(String out_trade_no) {
        ...
        if (response.isSuccess()) {
            ...
            resultMap.put("total_amount", response.getPayAmount());
            resultMap.put("payment_time", response.getSendPayDate());
        } else {
    ...
    @Override
    public void updatePayment(PaymentInfo paymentInfo, String total_amount, Date paymentTime) {
        // 幂等性检查
        ...
        if (StringUtils.isNotBlank(paymentInfoResult.getPaymentStatus()) && paymentInfoResult.getPaymentStatus().equals("已支付")) {
            ...
            try {
                ...
                mapMessage.setString("total_amount", total_amount);
                //mapMessage.setObject("payment_time", paymentTime);
                producer.send(mapMessage);
                session.commit();
            } catch (Exception ex) {
                try {
